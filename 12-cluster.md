...
Go ahead and open your terminal and log in to ARCHER2. 
Remember, we are opening a terminal on our local laptop to connect to a remote 
system which is physically at EPCC in Edinburgh. 
...
This was covered in [lesson 1](10-connecting-hpc.Rmd) or you can find a QuickStart 
...
We can check where we are in the system with the `hostname` command. (You
...
*Hint:* The shell commands `pwd`, `cd` and `ls` may come in handy.
...
head node (because we will get in trouble otherwise).
...
system. While this storage can be
...
This is an important point to remember: files saved on one login node are available 
in `/home` regardless of which login node you are on.
