...
```bash 
rsync -avzP -e "ssh -i ~/.ssh/key_for_remote_computer" path/to/local/dir `r config$remote$user`@`r config$remote$login`:directory/path/on/`r config$remote$name`/
```
:::
...
```bash
`r config$local$prompt` ssh `r config$remote$user`@`r config$remote$login`
`r config$remote$prompt` tar -tf hpc-intro-data.tar.gz
hpc-intro-data/
...
```bash
`r config$remote$prompt` du -sh hpc-intro-data.tar.gz
...
```bash
`r config$remote$prompt` tar -xvzf hpc-intro-data.tar.gz
```
```output
hpc-intro-data/
hpc-intro-data/north-pacific-gyre/
...
```bash
`r config$remote$prompt` du -sh hpc-intro-data
144K    hpc-intro-data
```
:::
